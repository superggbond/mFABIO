...
run_mfabio <- function(
  ...
  # --- 2. Enhanced Initialization (with fixed intercept) ---
  if (verbose) cat("Initializing VB parameters...\n")
  
  # Better initialization for intercept (now fixed effect)
  y_prop <- mean(y)
  mu_intercept <- qnorm(max(0.01, min(0.99, y_prop)))
  ...
  # --- 3. Optimized VB Iterations (with fixed intercept) ---
  iter <- 0
  converged <- FALSE
  divergence_count <- 0
  max_divergence <- 5
  ...
  for (iter in 1:max_iter) {
    ...
    # --- Update PC effects ---
    if (include_pleio_term && K > 0) {
      residual_alpha <- E_z - mu_intercept - G_E_beta_term
    ...
    # --- Compute ELBO if requested (modified for fixed intercept) ---
    if (check_elbo && iter %% elbo_frequency == 0) {
      ...
      # Note: No KL term for intercept since it's fixed
